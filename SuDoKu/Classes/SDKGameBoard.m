...
static SDKDifficulty level = SDKDifficultyNone;
...
void SDKRandomizeEmptyGrid(int aGrid[9][9])
{
    ...
    switch (level)
    {
        case SDKDifficultyEasy:
            count = 32;
            break;
        case SDKDifficultyNormal:
            count = 48;
            break;
        case SDKDifficultyHard:
            count = 58;
            break;
        case SDKDifficultyExtreme:
            count = 68;
            break;
        case SDKDifficultyNone:
            count = 9;
            break;
...
#pragma mark - SDKGameBoard

@implementation SDKGameBoard
...
{    
    ...
    if (self)
    {
        ...
        mainGrid = [[SDKMainGrid alloc] initWithFrame:CGRectMake(gridSize/2-(gridSize/2-offsetX), offsetY, gridSize, gridSize+48)];
        [self addSubview:mainGrid];
        
        timerField = [[SDKTimerField alloc] initWithFrame:CGRectMake(mainGrid.frame.origin.x+mainGrid.frame.size.width-(mainGrid.frame.size.width/2), mainGrid.frame.origin.y-(timerHeight+4), mainGrid.frame.size.width/2, timerHeight)];
        ...
        [backButton addTarget:[SDKMainViewController sharedController] action:@selector(doneGame:) forControlEvents:UIControlEventTouchDown];
        ...
        selectionCase = [[SDKSelection alloc] initWithFrame:CGRectMake(0, 0, mainGrid.frame.size.width/9, mainGrid.frame.size.height/9)];
        [selectionCase setHidden:YES];
        [self addSubview:selectionCase];
        
        numGrid = [[SDKNumberGrid alloc] initWithFrame:CGRectMake(0, 0, numGridSize.width, numGridSize.height+numGridSize.height/3)];
...
- (id)initWithFrame:(CGRect)frame :(int)num{
    self = [super initWithFrame:frame];
    ...
    if (self)
    {
        ...
        moveTimerField = [[SDKTimerField alloc] initWithFrame:CGRectMake(mainGrid.frame.origin.x+mainGrid.frame.size.width-(mainGrid.frame.size.width/2)-130, mainGrid.frame.origin.y-(timerHeight+4), mainGrid.frame.size.width/2, timerHeight)];
...
- (void)setPlayer2Grid:(BoardArray)board{
    ...
    mySolver = [[SDKSolver alloc] initWithArray:board.solutionArray];
...
- (void)setNewGrid:(SDKDifficulty)diff;
{
    ...
    mySolver = [[SDKSolver alloc] initWithNewGrid];
...
- (void)addNumberOnGrid:(int)num
{
    ...
    if ([self lastNumberAdded])
    {
        if ([self solvePuzzle])
        {
            ...
            SDKAlertView *alert = [[SDKAlertView alloc] initWithTitle:NSLocalizedString(@"Congratulations !",@"End Alert Title") message:[NSString stringWithFormat:@"%@ %1.0f %@",NSLocalizedString(@"The puzzle has been solved in",@"Puzzle solved"),gameTime, NSLocalizedString(@"seconds",@"Seconds")] delegate:self cancelButtonTitle:NSLocalizedString(@"Review",@"Review") otherButtonTitles:NSLocalizedString(@"Main Menu",@"Main Menu"), nil];
            alert.tag = 1;
            [alert show];
            //[alert release];
        }
        else
        {
            if (!bHints)
            {
                SDKAlertView *alert = [[SDKAlertView alloc] initWithTitle:NSLocalizedString(@"Solution",@"Solution Alert Title") message:NSLocalizedString(@"The puzzle has not been solved",@"Puzzle not solved") delegate:self cancelButtonTitle:NSLocalizedString(@"Hints",@"Hints") otherButtonTitles:NSLocalizedString(@"Main Menu",@"Main Menu"), nil];
                alert.tag = 2;
                [alert show];
                //[alert release];
            }
...
- (void)addNumberOnGrid:(int)num :(int)playerNum :(int)xThing :(int)yThing :(int)score
{
    ...
    if(playerNum == player){
        ...
        [[SDKMainViewController sharedController] sendMove:x :y :num :player :turnNumber :score];
    }
...
{
    SDKAlertView *alert = [[SDKAlertView alloc] initWithTitle:NSLocalizedString(@"Start over",@"Reset Grid Alert Title") message:NSLocalizedString(@"Would you like to reset the grid?",@"Reset Grid Alert Message") delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel",@"Cancel") otherButtonTitles:NSLocalizedString(@"OK",@"OK"),nil];
    alert.tag = 0;
    [alert show];
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 1)
    {
        if (buttonIndex == 1)
        {
            ...
            [timerField restart];
            [[SDKMainViewController sharedController] doneGame:alertView];
        }            
    }
    else if (alertView.tag == 2)
    {
        ...
        if (buttonIndex == 1)
        {
            ...
            [[SDKMainViewController sharedController] doneGame:self];
        }
        else
        {
            [self giveHints];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        mainLines = malloc(sizeof(SDKLine) * 4);
        ...
        for (int i = 0; i < 9; i++)
        {
            SDKGrid *sdkGrid = [[SDKGrid alloc] initWithFrame:CGRectMake(origin[i].x, origin[i].y, w, h)];
...
#pragma mark - SDKNumberGrid

@implementation SDKNumberGrid
...
#pragma mark - SDKSelection

@implementation SDKSelection
...
#pragma mark - SDKTimerField

@implementation SDKTimerField
...
- (void)timerFireMethod:(NSTimer *)theTimer
{
    if (self.isRunning)
    {
        ...
        if (gameTime > 32400)
        {
            SDKAppDelegate *delegateApp = (SDKAppDelegate *)[[UIApplication sharedApplication] delegate];
